 ...
 * these are the library's — hence the correction in `getMinimapMarks`. The
...
/**
 * A file's marks, corrected against the height CodeView actually laid out — so
 * wrapped lines, or a library change to a row or bar, squash the marks along
 * with the file rather than sliding them out of it.
 */
export const getMinimapMarks = (
	file: MinimapFile,
	block: { top: number; height: number },
): Array<MinimapMark> => {
	if (file.contentHeight <= 0) return [];

	const scale = block.height / file.contentHeight;

	return file.marks.map((mark) => ({
		...mark,
		top: block.top + mark.top * scale,
