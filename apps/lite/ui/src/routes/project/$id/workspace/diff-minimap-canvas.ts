...
import { getMinimapMarks, type MinimapFile, type MinimapGeometry } from "./diff-minimap.ts";
...
const resolveLanes = ({
	...
	for (const [index, file] of files.entries()) {
		...
		for (const mark of getMinimapMarks(file, block)) {
			const lane = lanes[mark.side];
			const lineHeight = (mark.height * scale) / mark.widths.length;
			let y = mark.top * scale;
